...
def main_operator(input):
    ...
    if input[0] == "register_user":
        return g_instance2.register_user()

    elif input[0] == "register_app":
        c= g_instance2.register_app(input[1], input[2])
        return c

    #user has sessionid + appid from app and wants to login
    elif input[0] == "login_with_credential":
        result= g_instance2.login_with_credential(input[1],input[2])
        return result
    elif input[0] == "get_user_id":
        pass
        return g_instance2.return_logged_user_id_to_app(input[1],input[2])

    elif input[0] == "app_secret_code":
        return g_instance2.check_app_secret_code(input[1],input[2])
...
############################################## User##################################################
    ...
    #code working well
    def login_with_credential(self,sessionid,appid):
            ...
            if result == []:
                ...
                g_instance=g()
                g_instance.login_with_credential(sessionid,appid)
            if result!= []:
...
################################################ App ##################################################
    ...
    def register_app(self,appname, url):
        a=appname
        b=url
        g_instance=g()
        c=g_instance.generate_random_string()
...
########################################## check app secret code #############################################
    ...
    def check_app_secret_code(self,app_secret_code,app_code):
...
########################################## return logged userid to app #######################################
    ...
    def return_logged_user_id_to_app(self,session_id,app_id):
